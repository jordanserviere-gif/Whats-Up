...
| **Le halo urbain peint** | ⚠️ **le clair de lune ne l'est plus** : il passe desormais par une table de ciel calculee avec la Lune pour source, mise a l'echelle par `moonToSunIrradianceRatio`. Verifie par deux chemins independants — 0,267 lx d'eclairement au zenith contre 0,25 a 0,3 publies, et **6,65·10⁻³ cd/m² de brillance de ciel** contre 2,7 a 6,8·10⁻³ (19 a 18 mag/arcsec²). Reste **le halo urbain** : c'est une emission renvoyee par l'atmosphere, que decrit le modele de Garstang (1989), et il n'est pas implemente. Sa magnitude, elle, est deja ancree — `lightPollutionLux` par classe de Bortle — c'est sa **distribution angulaire** qui manque. |
| **Spectre du clair de lune** | suppose **identique au spectre solaire**. L'albedo de la Lune croit avec la longueur d'onde : le clair de lune reel est un peu plus rouge que la lumiere du Soleil, d'environ 1,2 en rapport rouge sur bleu. Le rapport `moonToSunIrradianceRatio` etant photometrique — un rapport de lux — la **luminance** est juste et seule la **teinte** est en defaut. Un albedo spectral lunaire tabule le corrigerait. |
| **Les etoiles sont eteintes deux fois** | ⚠️ **trouve en rendant le clair de lune physique, mais anterieur a lui.** Deux mecanismes suppriment les etoiles sous un ciel clair : `pointIntensity(magnitude, limitingMagnitude)`, qui les efface pres de la magnitude limite photometrique, et l'exposition adaptative, qui comprime toute la scene. La photometrie est juste — 3,57 de magnitude limite sous pleine Lune contre 4,0 publie, 6,59 sans Lune contre 6,5 — mais le rendu en montre nettement moins que trente sur un champ de quatre-vingts degres. Tant que l'exposition ne bougeait pas la nuit, le double comptage restait invisible ; il ne l'est plus. Le correctif est de decider **lequel des deux possede** la visibilite d'une etoile : la chaine d'affichage est physique, donc c'est elle, et la magnitude limite ne devrait plus servir qu'aux etiquettes et au tri du catalogue. |
| **Carte d'ombre du relief** | 512 texels sur ±60 km font **234 m par texel**, huit fois plus grossier que le relief. ⚠️ **J'en avais conclu que c'etait le maillon faible visible ; la mesure l'a refuse.** Une seconde carte dix fois plus fine, heritant de la premiere a sa frontiere, a ete construite puis **retiree** : zero point sur dix mille change de verdict sur le banc, 0,00 a 0,01 % des pixels sur trois vues du relief reel. Ce qui a ete garde est la **tolerance verticale du test d'ombre**, qui valait 300 m poses et vaut desormais un demi-texel deduit, 117 m — seul changement mesurable, 0,92 % des pixels sur une vue au lever. Trois causes restent en lice et non departagees : relief lisse a cette echelle, sol proche rarement a l'ecran depuis un sommet, cadrages insuffisants. **Un texel grossier n'est pas une erreur visible** : le rapport de 1209 pixels a cinq kilometres est une propriete de la carte, pas un ecart a l'image juste. |
| **Lumieres urbaines** | le sol **emet** desormais la nuit, la ou OSM montre du bati et de la voirie. La quantite vient de l'**EN 13201-2** — luminance de chaussee eclairee, 1,0 cd/m² en classe M3 urbaine — et la couleur d'un **spectre de Planck a 2700 K remis a l'echelle sur cette luminance**, converti par le meme operateur spectral que le Soleil : couleur et intensite sortent ensemble, sans constante d'ajustement. ⚠️ Trois approximations subsistent. **La temperature est un choix** dans l'intervalle de l'arrete du 27 decembre 2018 — plafond 3000 K, 2700 K en peripherie de parc, 2400 K en coeur, sodium ancien vers 2000 K. **Un Planck n'est pas un spectre de lampe** : le sodium emet des raies, un LED un pic bleu et un phosphore ; la couleur est reproduite, la structure non, et l'extinction depend de la longueur d'onde. **La carte dit *bati*, pas *eclaire*** : une route de montagne y brille autant qu'une rue. Mesure au Ventoux, dont OSM connait la route du sommet : le premier plan rendait 29,9 niveaux quand le Causse Mejean desert rendait 0. Un exposant de densite applique au flou — **1,6**, pose — separe le trait isole du tissu dense, mais aucune valeur ne separe proprement, et des villages disparaissent avec la route. Une carte de radiance nocturne mesuree, VIIRS, rendrait cette correction inutile. **Le seuil d'allumage** est photometrique — cinq a cent lux, la dispersion d'un parc et non le seuil d'une cellule, qui donnerait une bascule d'un degre. **La lueur dans l'air** suppose une diffusion isotrope, un albedo de diffusion unite et un plan emetteur infini. |
| **Drape orthophotographique** | l'orthophoto de l'IGN donne au sol sa **couleur reelle**, et **elle seule** : une orthophoto est une image deja eclairee, et l'employer telle quelle ferait compter la lumiere deux fois — ombres de midi cuites sous un Soleil couchant. On la ramene donc a luminance unite et l'on n'en garde que les rapports de couleur, le moteur conservant toute la brillance. Un controle tient la propriete. ⚠️ **Ce qu'on jette avec la luminance, ce sont les vrais ecarts d'albedo** — une foret a 0,08 contre un calcaire a 0,35 — qui vivent precisement dans la clarte de l'image. Les recuperer demanderait de separer la structure fine, qui est de la couverture du sol, du degrade large, qui est l'eclairage du jour de la prise de vue. ⚠️ **France seulement**, et vingt-huit kilometres seulement. Mesure depuis Carpentras : 100 % des pixels changent, chrominance +3,0 %, luminance +6 niveaux sur 255 — cette derniere etant attendue, un albedo decale vers le vert renvoyant moins sous une lumiere rougie. |
| **Objets du ciel profond** | ⚠️ **resolu.** Le calque etait le seul du moteur sans terme de transport, et il avait sa propre loi de visibilite — un contraste decale de 1,5 puis divise par 3,5, plafonne, multiplie par 0,42 : le coeur de M31 rendait **169 niveaux sur un ciel a 0**. Trois constantes inventees, supprimees en separant deux questions distinctes. **Detecter** profite de la sommation spatiale de l'oeil, `mu − 2,5·log10(Omega_eff)` avec `Omega_eff = min(Omega_oeil, Omega_objet)` — le plafond est obligatoire, et fait retomber tout objet plus petit que l'aire de sommation sur sa magnitude integree, si bien que la loi des sources ponctuelles en devient la limite plutot qu'un cas separe. **Paraitre brillant** n'en profite pas : l'image retinienne d'une source etendue garde sa brillance de surface. L'aire de sommation est **deduite** de M33 au seuil sous un ciel vierge — 32,9 arcmin, dans l'intervalle publie sans y avoir ete prise — et verifiee sur M42, M13, M81 et M101, qui se classent comme les observateurs les classent ; M81 est le controle, meme brillance de surface que M42 et pourtant hors de portee. M31 passe de 169 a **32 niveaux**. **Le zoom est un instrument**, et il se deduit : la plus petite ouverture capable de resoudre un pixel affiche, par le critere de Rayleigh, la bascule tombant d'elle-meme a 4,99° de champ la ou cette ouverture depasse la pupille adaptee. ⚠️ Un capteur n'ayant pas de batonnets, la loi mesopique se retire dans la proportion de lumiere que l'oeil seul n'aurait pas collectee. La **couleur** ne vient plus d'un jeton d'interface mais de l'indice `B−V` du catalogue, calibre pour 87 % des objets — et la cale `radianceFromDisplay`, qui inverse la courbe canal par canal, a du etre restreinte a la luminance : elle transformait 1 : 0,91 : 0,83 en **1 : 0,26 : 0,15** des que l'opacite baissait. Les 548 objets portent une **vraie image** DSS2, tuiles de 96 a 384 pixels selon leur taille, 4,35 Mo ; l'octet stocke un **ecart de magnitude**, jamais une radiance. Orientation mesuree, `−PA`, ecart median 4,0° sur 154 objets allonges ; relecture des octets livres, moyenne du profil a 0,8 % pres. ⚠️ **Restent** : le profil non lisse a l'echelle de sommation, **aucun temps de pose** — ce qui manque pour colorer une nebuleuse depuis un ciel de ville — un seul filtre, `B−V` qui suppose un corps noir, et les etoiles de champ comptees deux fois. |
| **Micro-relief du sol** | ⚠️ **le motif est invente**, et aucune bosse qu'il produit n'existe : c'est une texture procedurale appliquee aux seules normales, pour qu'une plaine cesse de rendre un aplat de lumiere uniforme. Son **amplitude**, elle, est mesuree : pente efficace de 0,012 en Beauce, 0,017 en Camargue et dans les Landes, 0,087 dans la vallee du Rhone, 0,291 au Ventoux — relevees sur tuiles RGE ALTI a 3,4 m. La valeur retenue, 0,06, tombe dans l'intervalle des plaines. **Elle reste un choix.** L'extrapolation sous la resolution du modele numerique s'appuie sur un exposant de Hurst mesure a 0,980, soit une pente invariante d'echelle sur un facteur huit — statistiquement licite, geographiquement fausse. Aucun deplacement de geometrie : la ligne d'horizon n'est pas touchee. |
| **Pente caracteristique du relief** | `CHARACTERISTIC_SLOPE = 0,2`, soit onze degres, sert a convertir un saut de distance en erreur angulaire dans `rangeStepsFor` : deux anneaux distants de `Δd` peuvent sauter une crete de `pente × Δd`. ⚠️ **C'est la seule valeur posee du module d'echantillonnage**, et elle n'est pas mesuree. Une rugosite par region, calculee avec la pyramide, la remplacerait — c'est ce que fait une erreur geometrique au sens strict de la SSE, qui porte sur l'ecart en metres entre representation et terrain, non sur la taille d'une cellule. |
| **Dette de l'axe des distances** | ⚠️ **la silhouette est corrigee, la surface ne l'est pas.** Chaque anneau prend desormais le point le plus haut de sa tranche de profondeur, et non l'altitude a sa distance : les tranches partitionnant la portee, le maximum sur leur union est le maximum sur le rayon — l'operation qu'effectue un logiciel de panorama. Erreur de ligne d'horizon depuis le pic Cassini : quadratique de **0,130° a 0,067°**, et de 0,07–0,18° a 0,004–0,023° sur quatre azimuts sur cinq. ⚠️ L'enveloppe ne s'applique **qu'au-dela de 130 km**, la ou la surface n'a plus dix pour cent de son contraste : appliquee partout elle changeait le relief proche en plateaux plats separes de falaises, et relevait a tort les silhouettes portees par une crete proche. Il reste que **le maximum borne l'erreur sans l'annuler** : un sommet plus etroit que l'ecart entre deux sondages est rabote, chiffre a 43 % sur une bosse de synthese. Et la **surface** entre les cretes, elle, reste echantillonnee au pas des anneaux — 173 px d'erreur a 2° de champ contre 16 vises. Tenir la cible demanderait quatre mille anneaux contre quatre cent seize : c'est la limite de la representation radiale, que seuls un quadtree ou une marche par pixel franchiraient. |
| **Azimut geodesique** | ⚠️ **piege de mesure, pas defaut du moteur.** Calculer un azimut en projetant les differences de latitude et de longitude a plat donne **1,30° d'erreur** sur Cassini → mont Blanc, soit 6,5 km de cote a 286 km. Le moteur, lui, place le relief par Vincenty direct sur l'ellipsoide et n'a pas ce probleme ; mais toute mesure externe doit passer par `vincentyInverse`, faute de quoi elle compare deux montagnes differentes. |
| **Couture du globe à zéro degré** | une ligne sombre horizontale traverse l'image à **0° d'élévation exact**, visible sur toutes les vues lointaines. Ce n'est pas l'horizon apparent — à −1,21° depuis 1700 m — mais le bord de la calotte du globe, dont la géométrie s'ouvre à l'horizontale. Le découpage par pixel taille bien le bord réel, mais la jointure des deux surfaces reste lisible. |
| **Couture entre niveaux de pyramide** | ⚠️ **predite, puis cherchee et non trouvee.** Le pas de la donnee saute d'un facteur quatre a la frontiere — 27,4 m a 109,9 m — et l'on attendait un anneau de perte de nettete une fois le maillage assez fin pour le resoudre. Deux cadrages au Ventoux, azimut 20° ou la frontiere tombe a **29,8 km** en distance de Tchebychev, champs de 2° et 0,8° : rien de visible. Deux raisons plausibles, non departagees : le fondu etale le saut sur 3,6 km de profondeur, et l'extinction a deja ramene le contraste a 60 % a cette distance. Conclusion : etendre le niveau fin ne vaut pas ses 256 tuiles aujourd'hui. |
| **Résolution du relief proche** | ⚠️ **corrigée en France.** La source mondiale plafonne à 30 m ; le RGE ALTI de l'IGN descend à 3,4 m, et la mesure sur la tuile du Ventoux montre du relief à toutes les échelles jusque-là — dénivelé type 7,81 m à 27 m d'écart, **1,01 m à 3,4 m**. Un champ proche de 7 km à 3,42 m, stocké au quart de mètre, se compose avec la pyramide. 64 % des pixels changent. **Hors de France, rien ne change** : le service répond 404 et l'on reste à 30 m. ⚠️ Et le gain n'est pas encore visible : à 3,7 km deux anneaux du maillage sont distants de 124 m quand la donnée en décrit un tous les 3 m — voir la dette de l'axe des distances. |
| **Précision verticale du RGE ALTI** | la grille est métrique, la vérité ne l'est pas partout. Lidar en plaine et sur les côtes, à 20–50 cm près ; **radar en montagne** — Alpes, Pyrénées, Cévennes, Corse — où sur les fortes pentes l'erreur atteint **7 m**. Le Ventoux est dans cette seconde catégorie. On gagne donc surtout de la **texture** à petite échelle, pas de l'exactitude altimétrique. |
