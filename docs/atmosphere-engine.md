...
| **Objets du ciel profond** | ⚠️ **resolu.** Le calque etait le seul du moteur sans aucun terme de transport : une galaxie brillait autant a l'horizon qu'au zenith. L'extinction s'applique desormais a la brillance de surface comme a une magnitude — `mu + k·X` — et le contraste de M31 sur un ciel a 21,5 passe de −0,78 au zenith a −3,36 a cinq degres. Sa couleur venait d'un **jeton d'interface**, un par type d'objet ; elle passe maintenant par la loi mesopique deja appliquee aux etoiles. La conversion brillance de surface → luminance n'introduit aucune constante et se recoupe a 0,8 % pres sur la brillance publiee d'un ciel noir. Les 548 objets de plus de cinq minutes d'arc portent desormais une **vraie image** : un atlas de profils DSS2 construit hors ligne depuis hips2fits, 2,58 Mo. ⚠️ Ce n'est **pas** une carte de radiance — l'octet stocke un **ecart de magnitude** a la brillance moyenne, `mu + 5 − 10·q`, ce qui laisse la photometrie du moteur decider du niveau absolu. L'orientation a ete mesuree et non supposee : c'est `−PA` qui redresse le grand axe, ecart median 3,1° sur 154 objets allonges. Relecture des octets livres : la moyenne du profil s'ecarte de 0,9 % au pire. ⚠️ **Restent** : un seul filtre (rouge), les etoiles de champ comptees deux fois, l'ellipse du catalogue qui coupe l'objet, 96 pixels par tuile, et une teinte encore issue d'un jeton d'interface — que le plafond scotopique masque presque toujours. |
