...
| **Objets du ciel profond** | ⚠️ **resolu.** Le calque etait le seul du moteur sans aucun terme de transport : une galaxie brillait autant a l'horizon qu'au zenith. L'extinction s'applique desormais a la brillance de surface comme a une magnitude — `mu + k·X` — et le contraste de M31 sur un ciel a 21,5 passe de −0,78 au zenith a −3,36 a cinq degres. Sa couleur venait d'un **jeton d'interface**, un par type d'objet ; elle passe maintenant par la loi mesopique deja appliquee aux etoiles. La conversion brillance de surface → luminance n'introduit aucune constante et se recoupe a 0,8 % pres sur la brillance publiee d'un ciel noir. ⚠️ **Il reste que les objets sont des taches procedurales** : un noyau gaussien et un halo, pas la structure reelle. Un atlas d'images hips2fits construit hors ligne la fournirait — la source est ouverte et testee, 544 objets depassent cinq minutes d'arc. |
